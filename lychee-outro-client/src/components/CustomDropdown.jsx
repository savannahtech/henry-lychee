...
export default function CustomDropdown({selectedAction, setSelectedAction}) {
  const actions = ["Listen on Spotify", "Listen on Apple", " Listen on Google"];
  return (
    <div className="">
      <Menu as="div" className="relative inline-block text-left">
        <div>
          <Menu.Button className="inline-flex w-[170px] justify-between rounded-md 0 px-3 py-[6px] text-sm border-gray-300 border-[1px] font-medium text-black">
        ...
        <Transition as={Fragment} enter="transition ease-out duration-100" enterFrom="transform opacity-0 scale-95" enterTo="transform opacity-100 scale-100" leave="transition ease-in duration-75" leaveFrom="transform opacity-100 scale-100" leaveTo="transform opacity-0 scale-95">
          <Menu.Items className="absolute right-0 mt-2 w-56 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black/5 focus:outline-none">
            {actions?.map((action, index) => (
              <div className="p-[3px]" key={index} onClick={() => setSelectedAction(action)}>
                <Menu.Item>{({active}) => <button className={`${active ? "bg-violet-500 text-white" : "text-gray-900"} group flex w-full items-center rounded-md px-2 py-2 text-sm`}>{action}</button>}</Menu.Item>
              </div>
            ))}
          </Menu.Items>
        </Transition>
